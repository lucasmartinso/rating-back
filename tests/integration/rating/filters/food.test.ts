...
describe('TEST GET /places/food/:type', () => { 
    it('Should answer 200, send type best', async() => { 
        ...
        const { status, body }: { status: number, body: any } = await server.get('/places/food/best').send({});

        expect(status).toBe(httpStatus.OK);
        expect(body).toBeInstanceOf(Array);
        expect(Number(body[0].food)).toBeGreaterThanOrEqual(Number(body[1].food));
    });
    ...
    it('Should answer 200, send type last', async() => { 
        ...
        const { status, body }: { status: number, body: any } = await server.get('/places/food/last').send({});

        expect(status).toBe(httpStatus.OK);
        expect(body).toBeInstanceOf(Array);
        expect(Number(body[0].food)).toBeLessThanOrEqual(Number(body[1].food));
    });
});
