...
describe('TEST GET /places/environment/:type', () => { 
    it('Should answer 200, send type best', async() => { 
        ...
        const { status, body }: { status: number, body: any } = await server.get('/places/environment/best').send({});

        expect(status).toBe(httpStatus.OK);
        expect(body).toBeInstanceOf(Array);
        expect(Number(body[0].environment)).toBeGreaterThanOrEqual(Number(body[1].environment));
    });
    ...
    it('Should answer 200, send type last', async() => { 
        ...
        const { status, body }: { status: number, body: any } = await server.get('/places/environment/last').send({});

        expect(status).toBe(httpStatus.OK);
        expect(body).toBeInstanceOf(Array);
        expect(Number(body[0].environment)).toBeLessThanOrEqual(Number(body[1].environment));
    });
});
