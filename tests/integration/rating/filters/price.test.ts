...
describe('TEST GET /places/price/:type', () => { 
    it('Should answer 200, send type best', async() => { 
        ...
        const { status, body }: { status: number, body: any } = await server.get('/places/price/best').send({});

        expect(status).toBe(httpStatus.OK);
        expect(body).toBeInstanceOf(Array);
        expect(Number(body[0].price)).toBeGreaterThanOrEqual(Number(body[1].price));
    });
    ...
    it('Should answer 200, send type last', async() => { 
        ...
        const { status, body }: { status: number, body: any } = await server.get('/places/price/last').send({});
